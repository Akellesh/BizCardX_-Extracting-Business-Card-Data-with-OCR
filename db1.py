...
for record in cur.fetchall():
    st.write(record['card_holder_name'],)

cur.close()
# selected_query = st.selectbox("Select Customer Details", options=run_query(cus_names))
# if st.button('View',selected_query) is not None:
#
#     rows = run_query("SELECT * FROM CUSTOMERS selected_query;")
#
#     # Print results.
#     for row in rows:
...
#         selected = option_menu(
#             menu_title="Manage Customer Data", options=["Upload and View Business card", "Update", "Delete"],
#             icons=["database-add", "database-dash"], menu_icon="database-gear",
#             default_index=0, orientation="horizontal")
#
#         if selected == "Upload and View Business card":
#
...
#                 view = st.button('View Customer Details')
#                 if view:
#                     with st.spinner('Converting details from Image...'):
#                         st.dataframe(df)
#
...
#
#                 # selected = option_menu(
#                 #     menu_title="Manage Customer Data", options=["Update", "Delete"],
#                 #     icons=["database-add", "database-dash"], menu_icon="database-gear",
#                 #     default_index=0, orientation="horizontal")
#
#         if selected == "Update":
